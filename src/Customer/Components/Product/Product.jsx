...
export default function Product() {
  ...
  const handleRadiopFilterChange=(val,sectionId)=>{
    const searchParams=new URLSearchParams(location.search);
    searchParams.set(sectionId,val);
    const query=searchParams.toString();
    navigate({search: `?${query}`})
  }
  ...
  useEffect(()=>{
    ...
    colorValue,sizeValue,priceValue,discountValue,sortValue,pageNumber,stock
  ])
                  ...
                  <div className="py-1">
                    {sortOptions.map((option) => (
                      <MenuItem key={option.name}>
                        <a
                          onClick={()=>handleRadiopFilterChange(option.href,"sort")}
                          // href={option.href}
            ...
            <div className="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-5">
              ...
              <form className="hidden lg:block">
                ...
                <div className='flex justify-between items-center py-2'>
                  <h1 className='font-semibold text-xl text-gray-500'>Filter</h1>
                  <FilterAltOutlined/>
                </div>
                        ...
                        <FormControl>                             
                          ...
                          {section.options.map((option, optionIdx) => (
                            <div key={optionIdx} className='flex items-center'>                         
                                <FormControlLabel onChange={(e)=>handleRadiopFilterChange(e.target.value,section.id)} value={option.value} control={<Radio />} label={option.lable} />                             
                            </div>
                          ))}
                         </RadioGroup>
                        </FormControl>
